...
```{r}
p_chart <- pattern_chart(sa_object, sa_object["patterns"],
                         n.cutoff = 20,
                         c.cutoff = 17,
                         path = directory,
...
```
![Fig. 4.a: Itemsets corresponding to the mined patterns (except those of order 1.](./spectral_analysis_example/pattern_itemsets.png)
...
See that only patterns with an order value greater than 1 are plotted on this chart. The argument `length_one` allows to plot all patterns.
```{r}
p_chart <- pattern_chart(sa_object, sa_object["patterns"],
                         n.cutoff = 20,
                         c.cutoff = 16,
...
```{r}
# init(alt_sa_object, "nodes_links")
init(alt_sa_object, "patterns_links")
```
