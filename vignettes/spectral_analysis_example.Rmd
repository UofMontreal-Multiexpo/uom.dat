...
For a description of these data, you can use `help(oedb_sample)` to access the following help page:
...
2. For one inspection, research of the work situations described previously...
```{r}
...
4. Combination of points 2 and 3: search for the work situations described previously and one observation per situation not described (*JOB.TITLE*-*JOB.TASK* pair) concerning the remaining samples...
```{r}
...
# By prevention, assignment of a specific value to substances for which no family has been found
...
for (min_weight in seq(2, 5)) {
  spectrosome_w = spectrosome_chart(sa_object, "patterns", sa_object["patterns"],
                                    ...
                                    vertex_col = "categories",
                                    path = directory,
                                    name = paste0("spectrosome_of_patterns-weight", min_weight, ".png"),
                                    layout.par = NULL,
