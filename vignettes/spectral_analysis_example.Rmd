...
<center><b style='color:red;font-size:x-large'>
/!\\ OUTDATED REGARDING THE NOTION OF OBSERVATIONS; <br /> WRONG STATUS IN SPECTRAL ANALYZER EXAMPLE /!\\
</b></center>
...
![](./spectral_analysis_example/help.oedb_sample.png)
...
work_situations
```
These are work situations the user could identify beforehand. **The data frame must have a *WS.ID* column to identify the different work situations described.** The other columns correspond to the variables to use to describe a work situation and have the same names as the variables in the database used.

For this fictitious example, the variables named `JOB.TITLE` and `JOB.TASK` are used and random combinations were picked. Several couples with the same `WS.ID` are considered to correspond to the same work situation. Three work situations are described. The first one has one combination of the two variables. The second has two combinations which are considered as works with similar conditions. The last one is formed in the same way as the second one.
...
### 1.3.1. Association of each item to categories
...
families <- substances_information$SUBFAMILY[match(substances, substances_information$CODE)]
...
names <- substances_information$NAME[match(substances, substances_information$CODE)]
...
Use of the default values to create a spectral analyzer (see the help manual of the function `spectral.analyzer`)...
```{r}
...
View(sa_object)
```
![Fig. 1: View of the spectral analyzer object example.](./spectral_analysis_example/View_sa_object.png)

Some of these attributes will be saved in output files (see the following sections).

To have a description of the attributes, type `help("SpectralAnalyzer")`.
...
sa_object["nodes"]
```
![](./spectral_analysis_example/sa_object_nodes.png)

```{r}
sa_object["patterns"]
```
![](./spectral_analysis_example/sa_object_patterns.png)

Saving the nodes and the patterns in CSV files...
```{r}
# Saving the nodes
save_characteristics(sa_object, sa_object["nodes"],
                     file = paste0(directory, "sao.nodes.csv"), row.names = FALSE)
# Saving the patterns
save_characteristics(sa_object, sa_object["patterns"],
...
```{r}
spectrosome <- spectrosome_chart(sa_object, sa_object["nodes"],
                                 path = directory,
...
```{r}
save_characteristics(sa_object, spectrosome[["vertices"]],
                     file = paste0(directory, "spectrosome_of_nodes-vertices.csv"),
                     row.names = FALSE)

spectrosome[["vertices"]]
```
![](./spectral_analysis_example/nodes_vertices.png)
...
```{r}
item <- 3146
cluster <- cluster_chart(sa_object, sa_object["nodes"], item,
                         path = directory,
...
Saving vertex characteristics in a CSV file...
```{r}
save_characteristics(sa_object, cluster[["vertices"]],
                     file = paste0(directory, "node_cluster_of_", item, "-vertices.csv"),
                     row.names = FALSE)

cluster[["vertices"]]
```
![](./spectral_analysis_example/node_cluster_3146_vertices.png)
...
### 3.1.1. Pattern itemsets

Representation of the patterns as itemsets...
```{r}
p_chart <- itemset_chart(sa_object, sa_object["patterns"],
                         n.cutoff = 20,
                         c.cutoff = 17,
                         path = directory,
                         name = "pattern_itemsets.pdf",
                         title = "Pattern itemsets")
```
![Fig. 4.a: Itemsets corresponding to the mined patterns (except those of length 1.](./spectral_analysis_example/pattern_itemsets.png)
...
```{r}
save_characteristics(sa_object, p_chart,
                     file = paste0(directory, "pattern_itemsets.csv"),
                     row.names = FALSE)

p_chart
```
![](./spectral_analysis_example/pattern_itemsets_patterns.png)

See that only patterns with a length greater than 1 are plotted on this chart. The argument `length_one` allows to plot all patterns.
```{r}
p_chart <- itemset_chart(sa_object, sa_object["patterns"],
                         n.cutoff = 20,
                         c.cutoff = 16,
                         length_one = TRUE,
                         path = directory,
                         name = "all_pattern_itemsets.pdf",
                         title = "All pattern itemsets")
```
![Fig. 4.b: Itemsets corresponding to the mined patterns.](./spectral_analysis_example/all_pattern_itemsets.png)
...
```
![Fig. 5: Spectrum of patterns.](./spectral_analysis_example/spectrum_of_patterns.png)

Saving pattern characteristics in a CSV file...
```{r}
save_characteristics(sa_object, spectrum,
...
```{r}
spectrosome <- spectrosome_chart(sa_object, sa_object["patterns"],
                                 path = directory,
                                 name = "spectrosome_of_patterns.png",
                                 title = "Network of patterns")
```
![Fig. 6: Spectrosome of the patterns.](./output/spec_patterns/spectrosome_of_patterns.png)
...
```{r}
item <- 3146
cluster <- cluster_chart(sa_object, sa_object["patterns"], item,
                         path = directory,
...
cluster[["vertices"]]
```
![](./spectral_analysis_example/pattern_cluster_3146_vertices.png)
...
min_weight <- 3
patterns <- get_patterns(sa_object, sa_object["patterns"],
                         element = "weight",
                         value = min_weight,
                         condition = ">=")
...
patterns
```
![](./spectral_analysis_example/patterns_w3.png)
...
```
![Fig. 8: Spectrum of patterns with weigh filter.](./spectral_analysis_example/spectrum_of_patterns_w3.png)

Associated spectrosome...
```{r}
spectrosome <- spectrosome_chart(sa_object, patterns,
                                 path = directory,
...
patterns
```
![](./spectral_analysis_example/patterns_persistent.png)
...
```
![Fig. 10: Spectrum of patterns with stauts filter.](./spectral_analysis_example/spectrum_of_patterns_persistent.png)
...
patterns
```
![](./spectral_analysis_example/patterns_chrome_vertices.png)
...
spectrosome[["vertices"]]
```
![](./spectral_analysis_example/spectrosome_vertices_chrome_vertices.png)
...
patterns
```
![](./spectral_analysis_example/patterns_chrome_edges.png)
...
spectrosome[["vertices"]]
```
![](./spectral_analysis_example/spectrosome_vertices_chrome_edges.png)
...
## 4.4. Complex nodes and patterns

Let's take a look at another example which has a way more patterns (the creation of the concerning spectral analyzer object is not shown).

```{r eval = FALSE, echo = FALSE}
# No run, no show
# Creation of the SA object specific to this section
IMIS <- readRDS("IMIS_clean_20191107.RDS")
other_obs <- make_observations(IMIS,
                               by = c("COMPANY_INDEX", "JOB.TITLE", "YEAR"),
                               additional = "SUBST")

family_subst <- read.delim("Thesaurus_agents_clean_family_freq_07112019.txt", stringsAsFactors = FALSE)
...
other_substances <- sort(unique(unlist(sapply(other_obs, "[", "SUBST"))))
other_families <- family_subst$Family[match(other_substances, family_subst$Code)]
other_families[is.na(other_families)] <- "Unknown"
other_categories <- data.frame(item = other_substances, family = other_families)
for(i in seq_along(other_obs)) names(other_obs[[i]])[4] <- "CODE"

another_sa_object <- spectral.analyzer(other_obs, items = other_categories,
                                       target = "maximally frequent itemsets", count = 100)
```

```{r}
directory <- "./output/another_sa_object/"
dir.create(directory)

spectrosome_c <- spectrosome_chart(another_sa_object, another_sa_object["patterns"],
                                   vertex_size = "grouped", size_range = c(0.5, 1.5),
                                   displayisolates = FALSE, displaylabels = FALSE,
                                   path = directory, name = "without_isolates")
```

![Fig. 14: Spectrosome of patterns from another spectral analyzer, without displaying the isolates or the labels](./output/another_sa_object/without_isolates.png)

To focus on the patterns that make the junction between two or more families of substances (i.e. two values of one category), use the following...
```{r}
patterns <- get_complexes(another_sa_object, another_sa_object["patterns"],
                          category = "family", condition = "vertices")

spectrosome_c <- spectrosome_chart(another_sa_object, patterns,
                                   vertex_col = "categories",
                                   title = "Complexe patterns",
                                   path = directory, name = "complexe_patterns")
```

![Fig. 15: Spectrosome of patterns which are associated with more than one family of substances.](./output/another_sa_object/complexe_patterns.png)



# 5. Ad-hoc analysis and alternative initialization

Let's go back to our original example.

## 5.1. Processing on observations and items

### 5.1.1. Items, proportions and co-occurrences

Enumerate all substances in the list of observations...
```{r}
get_all_items(sa_object["observations"])
```

Count the number of occurrences of each substance (which is not necessarily the number of observations containing the substance)...
```{r}
# List of substances by observation (simply extract the "CODE" elements of each one)
substances_by_obs <- lapply(sa_object["observations"], "[[", "CODE")
# Total number of occurrences of each substance
table_on_list(substances_by_obs)
```

```{r}
# Number of involvement of each substance in multiple exposures
complexity_index(sa_object["observations"])
```

Compute some proportions...
```{r}
# Proportion of involvement of each substance in multiple exposures
complexity_ratio(sa_object["observations"])
```

```{r}
# Proportion of observations involving more than one substance
length(get_complex_obs(sa_object["observations"])) / length(sa_object["observations"])
```

Compute the co-occurrence matrix (for each pair of substances, the number of observations containing it)...
```{r}
cor_matrix <- co_occurrence_matrix(sa_object["observations"])
cor_matrix
```

And plot these co-occurrence values in two ways...
```{r}
# 1. A graph
co_occurrence_chart(sa_object, sa_object["items"], category = "family")
```
![](./spectral_analysis_example/co_occurrence_chart.png)

```{r}
# 2. A visualization of the matrix using the 'corrplot' package.
library(corrplot)
...
```{r}
corrplot.mixed(cor_matrix, is.corr = FALSE,
               upper = "color", tl.col = "black",
               order = "hclust")
```
![](./spectral_analysis_example/co_occurrence_matrix.png)

Plot a tree of the substances according to their family...
```{r}
category_tree_chart(sa_object, "family")
```
![](./spectral_analysis_example/category_tree_chart.png)

### 5.1.2. Filters on observations
...
View(get_obs_from_items(sa_object["observations"],
                        ...
                        presence = "all"))
```
![](./spectral_analysis_example/View_observations_25.png)

This way we can select observations containing a specific pattern...
```{r}
# Observations in which there is the itemset of the 2nd pattern
# (corresponding to the items having the codes 932 and 3180)
View(get_obs_from_items(sa_object["observations"],
                        items = sa_object["patterns"]$pattern[[2]],
                        presence = "all"))
```
![](./spectral_analysis_example/View_observations_pattern_2.png)
...
View(get_obs_from_info(sa_object["observations"],
                       info = list(JOB.TITLE = 44132001,
                                   JOB.TASK = "A8310")))
```
![](./spectral_analysis_example/View_observations_MT.png)

### 5.1.3. Search for items or information

Search for substances found in observations associated with a specific work situation...
```{r}
get_items_from_info(sa_object["observations"],
                    info = list(JOB.TITLE = 44121004,
                                JOB.TASK = "A5440"))
```

Search for work situations associated with a desired substance...
```{r}
...
Search for work situations associated with a specific pattern...
```{r}
# Search for the information "JOB.TITLE" and "JOB.TASK" associated to the 2nd pattern
# (corresponding to the items having the codes 932 and 3180)
get_info_from_items(sa_object["observations"],
                    items = sa_object["patterns"]$pattern[[2]],
                    info_names = c("JOB.TITLE", "JOB.TASK"))
...
spectrosome_1 <- spectrosome_chart(sa_object, sa_object["patterns"],
                                   vertex_col = "categories",
                                   nb_graphs = 3,
                                   path = directory,
...
![Fig. 16.a: Spectrosome in which vertices are colored according to the category associated with the items, example 1.](./output/alternatives/spectrosome_of_patterns-no_status-1.png)

![Fig. 16.b: Spectrosome in which vertices are colored according to the category associated with the items, example 2.](./output/alternatives/spectrosome_of_patterns-no_status-2.png)

![Fig. 16.c: Spectrosome in which vertices are colored according to the category associated with the items, example 3.](./output/alternatives/spectrosome_of_patterns-no_status-3.png)
...
for (min_weight in seq(2, 5)) {
  spectrosome_w <- spectrosome_chart(sa_object, sa_object["patterns"],
...
![Fig. 17.a: Spectrosome with the constraint of a minimum link weight of 2.](./output/alternatives/spectrosome_of_patterns-weight2.png)

![Fig. 17.b: Spectrosome with the constraint of a minimum link weight of 3.](./output/alternatives/spectrosome_of_patterns-weight3.png)

![Fig. 17.c: Spectrosome with the constraint of a minimum link weight of 4.](./output/alternatives/spectrosome_of_patterns-weight4.png)

![Fig. 17.d: Spectrosome with the constraint of a minimum link weight of 5.](./output/alternatives/spectrosome_of_patterns-weight5.png)


## 5.4. Working with association rules

### 5.4.1. Extracting association rules
...
2. Looking for rules whose union of the antecedent and the consequent form an entire pattern.
3. Looking for rules whose union of the antecedent and the consequent form an entire itemset among those defined in argument.
4. Looking for rules related to items specified in arguments as antecedent or consequent.
...
rules_1
```
![](./spectral_analysis_example/rules_1.png)
...
rules_2
```
![](./spectral_analysis_example/rules_2.png)
...
rules_3
```
![](./spectral_analysis_example/rules_3.png)
...
rules_4a
```
![](./spectral_analysis_example/rules_4a.png)
...
rules_4b
```
![](./spectral_analysis_example/rules_4b.png)
...
The `NULL` result means that no association rule exists between the substances *328* as antecedent and *3180* as consequent. In other words, the presence of the substance *328* implies the absence of the substance *3180* and the presence of the *3180* implies the absence of the *328*.

Furthermore, the confidence of a rule can be used to infer the confidence of the implication of the absence of the consequent, using the complement to 1:
```{r}
cat("Confidence of the rule 497 => 3345:    ", rules_4b[1, "confidence"])
cat("Confidence of the rule 497 => not 3345:", 1 - rules_4b[1, "confidence"])
```

### 5.4.2. Visualizing and saving association rules
...
save_characteristics(sa_object, rules_1,
                     file = paste0(directory, "rules.csv"), row.names = FALSE)
```

Association rules of length 2 can be plotted using the function `rules_chart`. If no rule is given as argument, all rules of length 2 are automatically extracted before being plotted:
```{r}
rc <- rules_chart(SA_instance, items = SA_instance["items"], category = "family")
rc$rules
```
...
plot(rc$graph)
```
![](./spectral_analysis_example/rules_chart.png)

### 5.4.3. Association rules as rules class object from the package arules

The argument `arules` of the function `extract_rules` allows to get rules as an object of class `rules` from the package `arules`. Such rules can be plotted using functions of the package `arulesViz` (see [arulesViz documentation](https://cran.csiro.au/web/packages/arulesViz/vignettes/arulesViz.pdf) for more visualizations).
```{r}
library(arulesViz)
a_rules <- extract_rules(sa_object, from = "observations", arules = TRUE)
plot(a_rules)
```



## 5.5. Alternative initialization and patterns

If some of the features are not interesting for a specific analysis and the amount of data implies a long object creation time, we may want to save time by not initializing some attributes.

For example, if spectrosomes and clusters of nodes do not matter for an analysis, we can manually initialize all attributes of a `SpectralAnalyzer` object except those concerning the links between the nodes:
```{r}
alt_sa_object <- spectral.analyzer(observations, items, init = FALSE)
init(alt_sa_object, "nodes")
```

```{r}
init(alt_sa_object, "patterns")
```

```{r}
# init(alt_sa_object, "node_links")
init(alt_sa_object, "pattern_links")
```

```{r}
is_init(alt_sa_object)
```

In this way the patterns can be obtained as `itemsets`, an object of a class from the package `arules`. See [arules documentation](https://cran.r-project.org/web/packages/arules/vignettes/arules.pdf) to know how to work with this class.

```{r}
itemsets <- init(alt_sa_object, "patterns", verbose = FALSE)
library(arules)
inspect(itemsets)
```
