...
Proposed as a tool for the assessment of occupational exposure to chemical agents, the **Maximum Cumulative Ratio approach** helps determine wheter a cumulative risk assessment is necessary.
...
An observation contain names, concentrations and identifiers of chemicals, identifiers and year of the sampling and extra information about the profession carried out for which the environment has been sampled. An observation looks like that:
```{r}
observations[2]
```
Here we can see an observation composed by 3 samples having the `SAMPLE.ID`s 18, 1 and 21 corresponding to the substances having the `CODE`s 57 and 77. The concentration values for these samples are respectively 0.007, 0.004 and 0.05.


Now we will separate the concentration values and name them with the substances they correspond to. We will have a list of named values which will have the same length as the list of observations:
```{r}
...
The analysis will be carried out on this set of concentration values.



## 1.2. Classification

Now let's take a look at the classification which will be used concerning the substances the values are from. This classification is the `TOXICITY` variable of the dataset `substances_information`. Here is a example of it:
```{r}
...
The dataset `substances_information` also contains the reference value of each substance in the variable `LIMIT`. However, some of them have no value for this variable. We will therefore set an arbitrary limit value for these variables without reference:
```{r}
...
We can see that the output data frame has 14 rows: one for each set of values. It has 7 columns for the 7 indicators:
...
The hazard and the maximum cumulative raio can be visualized by plotting $HI$ versus $MCR$ and displaying the percentage of the reciprocal of the maximum cumulative ratio, the elements producting the top hazard quotients and the associated MIAT groups. The function to use for this is `mcr_chart`.
...
**Note that functions presented in section [2.1.3. Compute a specific indicator](#compute-a-specific-indicator) cannot be used via this function.**
...
Before starting this section, we will name the observations of which concentration values are from, to better identify them:
```{r}
...
### 4.2.2. Subset corresponding to a specific class
...
```
We can see that only 7 observations contain substances related to the class of interest.

Let's take a look at the values from the 6th observation in which only two of the three values were kept:
```{r results="hold"}
