...
<tbody>
  ...
  <tr>
    ...
    <td>List of parameters for pattern search and characterization:<ul>
      ...
      <li>`count`: minimum number of occurrences that a pattern must appear to be kept when mining patterns.</li>
  ...
  <tr>
    <td>`patterns`</td>
    <td align="center">`data.frame`</td>
    <td>Set of mined patterns and characteristics of these patterns. Data frame containing the following 7 variables.<ul>
...
```
<center>
![](./transaction_analyzes/view_TA_instance.png){class=border_none}
</center>
...
* `count`: minimum number of occurrences that a pattern must appear to be kept when mining patterns.
...
* `"frequent itemsets"`: itemsets appearing in the transactions according to an occurrence threshold (defined by the parameter `count`).
...
The warning message appearing is generated by one the package used (*arules*) which informs that the choice of a minimum frequency of $1$ for the enumeration of the patterns risks requiring lots of memory in case where a lot of data is used and many patterns exist.
...
* `unique` specifies whether to return only unique values for each given itemsets. If duplicated values are removed, the result of each itemset is sorted. If they are removed, the correspondance between the returned values and the items forming the given itemsets is preserved.
...
The argumet `category` must be the name or number of one category associated with the items. The numbering is according to the order of the categories in the attributes `items_categories` linking the items to the categories. The argument `value` must be one of the levels of the related category.
...
* `get_patterns` extracts the patterns relating to specific items, categories or characteristics (year, length, frequency, weight, specificity or status).
...
* `"exact"`: the itemset corresponding to the node or the pattern must be exactly the same as the sought itemset.
...
In the last example, a data frame with no row is returned. That means no node itemsets are exactly the sought itemset.

#### Search by characteristic

Here are the possibles values of `element` to extract nodes or patterns according to one of their characteristics.
```{r}
...
* The **confidence**: quotient of the support of $X \cup Y$ and the support of $X$, i.e. the proportion of transactions in which the rule is correct relative to the number of transactions containing the antecedent $X$.
* The **lift**: quotient of the confidence of $X \rightarrow Y$ and the support of $Y$.
...
If the argument `itemsets` is `"patterns"` or a list of itemsets, the additional argument is `confidence`, defining the minimum confidence rules must have to be extracted (default `0.8`). No minimum support can be specify.
...
Co-occurrence charts are graphs in which vertices are items and edges are their number of co-occurrences in the transactions (i.e. for each pair of items, the number of transactions containig it). The method to use to plot such graphs is `co_occurrence_chart`. It works the same way as the one for objects of class `TransactionSet`, taking into account the item names and the values of one category.
...
* `ID` make the correspondance between the rows and the bars on the chart.
...
Association rules can be plotted using the method `rules_chart`. In this kind of chart, only rules of length $2$ are plotted, i.e. having one item as antecedent and another one as consequent. The chart is a graph in which vertices are items and edges are association rules. One characteristic of the rules is represented on the edges by coloring them: the support, the lift or the confidence. As the support and the lift from one rule to its reciprocal are equivalent, graphs representing these characteristics are undirected. Conversely, graphs representing confidence are directed.

The argument `display` defines which characteristic of the rules to represent on the graph.

* `"support"` specifies to represent the support of each rule.
* `"lift"` specifies to represent the lift of each rule.
* `"confidence"` specifies to represent the confidence of each rule. In this case, edges may be divided in two parts using two colors to represent the direction of the rules. The part of edge corresponding to the consequent of the rule is colored according to the confidence. The other part is colored according to the confidence of the reciprocal rule.
* `"highest confidence"` specifies to represent the confidences of the rules. Only the one having the highest confidence is represented between a rule and its reciprocal, unless they have the same confidence value.
* `"lowest confidence"` specifies to represent the confidences of the rules. Only the one having the lowest confidence is represented between a rule and its reciprocal, unless they have the same confidence value.
...
The argument `threshold` allows to specify a threshold above which the characteristic referred by the argument `display` must be for a rule to be considered.
...
Note that the argument `edge_alpha` can be used only when the argument `display` refers to the support or the lift.
...
Here is the list of available palettes if `display` refers to the confidence.
...
Here is the list of available palettes if `display` refers to the support or the lift.
...
The second palette argument allows to specify the direction: `palette_direction`. It can be $1$: from the lightest to the darkest; or $-1$: from the darkest to the lightest. It can be used whether to highlight higher values or lower values.
