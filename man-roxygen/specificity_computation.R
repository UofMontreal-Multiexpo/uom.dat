...
#'  \item{}{\mjeqn{H_{max} = \max log(U)}{H_max = max log(U)}}
#'  \item{}{\mjeqn{H_{min} = log \left( \frac{W_p}{W_p - U_p + 1} \right) + \left( \frac{U_p - 1}{W_p} \times log(W_p - U_p + 1) \right)}{H_min = log(W_p / (W_p - U_p + 1)) + (U_p - 1) / W_p * log(W_p - U_p + 1)}}
