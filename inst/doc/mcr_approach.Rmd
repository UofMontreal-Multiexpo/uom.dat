...
* The counts of each combination of the top two hazard quotients pairs for which the associated hazard index is greater than $1$, using the function `thq_pairs_freq`.
* The counts of each combination of the top hazard quotient with the associated MIAT group, using the function `thq_freq_by_group`.
...
For the first of the two functions (`thq_pairs_freq`), two other parameters exists:
...
If we want to turn the classification list into a binary matrix, we can use the function `turn_list_into_logical_matrix`. In such a matrix, `TRUE` indicates that a specific element is part of a specific class.
```{r}
classification_matrix <- turn_list_into_logical_matrix(classification_1)
classification_matrix
```

Conversely, we can use the function `turn_logical_matrix_into_list` to turn a classification matrix into a list:
```{r}
...
Finally, if we have a data frame or a matrix of characters in which two of the columns define the associations between the elements and the classes as in the following...
```{r}
...
classification_3
```
... we can use the functions `turn_data_frame_into_list` or `turn_char_matrix_into_list` to get the structure to be similar to the first one presented:
```{r}
...
Having our list of values which are concentration values for specific substances, we can call `turn_list_into_logical_matrix` to know which substances are presents in which sets of values. Each line refers to one observation (i.e. one set of values) and each column refers to one name found in the values of the list.
```{r}
...
This argument can be used in the functions `turn_list_into_char_matrix` and `turn_list_into_data_frame` as well.
