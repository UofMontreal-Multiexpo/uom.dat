...
Initially introduce in the field of data mining for market basket analysis, the notion of **transaction** correspond to a set of any elements, called **itemset**. In this historical example, the transactional dataset is the set of lists of purchase made in supermarkets in which the products are the items and any set of products is an itemset. The purpose was to find regularities in the shopping behavior of customers (or patterns) by finding the frequent itemsets to improve arrangement of products in shelves or in catalog, suggest other products, product bundling...

Nevertheless, this data mining approach can be applied to many other domains, such as the one in which the example we will use falls: health. Occupational health and safety research institutes use databases of exposure to chemicals in workplaces for the purpose of prevention of occupational accidents and diseases.
...
# 1. Structure of a set of transactions

## 1.1. Class TransactionSet
...
This transaction set is composed of 14 transactions. These transactions are lists in which the itemsets are stored in the element list named `"CODE"` referred as the **item key** and temporal data are stored in the element list named `"YEAR"` referred as the **year key**. These temporal data indicate when the transaction were made. Further data are associated with a transaction, accessible by the names `JOB.TITLE`, `JOB.TASK` and `SAMPLE.ID`.
...
* element named `"SAMPLE.ID"` refers to identifiers of atmospheric samples in specific workplaces;
* elements named `"JOB.TITLE"` and `"JOB.TASK"` refers to the job title and task of the people working in these workplaces;
...
* `unique_values` defines if duplicate values in aggregations must be removed;
...
There are 3 ways to group data with the function `make_OE_transactions`.
...
* One transaction corresponds to the samples of one inspection for one any combination of values of the variables used to identify one work situation (parameter `mode = 3`).
...
* Transactions (elements of the attribute `data`) can be accessed by numeric values.
* Replacing the attribute `names` renames the elements contained in each transaction if the given value is the same length as this attribute.
* Replacing the attribute `names` removes missing elements of the given value if it is smaller than this attribute.
...
Four methods allow to extract data from an object of class `TransactionSet`.
...
Here are examples of the first two methods.
```{r}
...
The method `get_info_from_items` is the opposite of the previous one: it finds data associated with specified items. The argument `presence` allows to specify if the transactions to focus on must contain all the sought items, any of them, or exactly the same itemset.
```{r}
...
Obviously, if no transaction contains the sought items, no related data can be found. Hence the `NULL` results in the last example.
...
Like for the method `get_info_from_items`, the method `get_trx_from_items` has an argument `presence` allowing to specify if the transactions to extract must contain all the sought items, any of them, or exactly the same itemset.
```{r}
...
As we can see with the last example, a `TransactionSet` can be composed of 0 transaction; the other attributes may still have interest.
...
The methods `get_complex_trx` and `get_simple_trx` are related to the itemset lengths. In other words, they are related to the number of items forming the transactions. Complex transactions are transactions having more than one item. Simple transactions are transactions having only one item.
```{r}
...
Co-occurrences between items in the transactions can be counted using the method `co_occurrence_matrix`. The resulting matrix contains the numbers of transactions containing each existing pair of items. The diagonal of the matrix is the number of transactions containing each single item.
...
Co-occurrence charts are graphs in which vertices are items and edges are their co-occurrences in transactions (i.e. for each pair of items, the number of transactions containing it). The method to use to plot such graphs is `co_occurrence_chart`.
...
Three arguments are related to the vertices. They correspond to their size, their transparency and the space between them and the incident edges: `vertex_size`, `vertex_alpha` and `vertex_margin`.
```{r}
co_occurrence_chart(TS_instance,
                    vertex_size = 4,
                    vertex_alpha = 0.4,
...
Two arguments are related to the labels next to the vertices. They correspond to their size and the space between them and the related vertices: `label_size` and `label_margin`.
```{r}
co_occurrence_chart(TS_instance,
                    label_size = 5,
                    label_margin = 0.15)
...
By the way, the chart being plotted with the packages `ggraph` and `ggplot2`, it can be modified or completed afterwards using the ggplot2 `last_plot` fuction or the returned object. We can for example change the plot limits to make truncated labels more visible.
```{r warning=FALSE}
# Loading the package
library(ggplot2)
...
Two arguments are related to the edges. They correspond to their looseness and their transparency: `edge_looseness` and `edge_alpha`. Looseness value close to $0$ implies straighter lines and value close to $1$ implies more curved lines.
```{r}
co_occurrence_chart(TS_instance,
                    edge_looseness = 0,
                    edge_alpha = 0.5)
...
Note that the RStudio graphics device may not be able to plot edges with transparency. In such a case, the edges will be actually displayed in the "Plot Zoom" window or while exporting the plot.

#### Palette

Two arguments are related to the palette used for coloring the edges. The first one allows to specify the sequential palette: `palette`. Here is the list of available palettes.
...
Here are two examples. See [the documentation of the package](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html) `corrplot` for more possibilities.
```{r fig.width=5}
...
For the object `TS_instance`, possible values which are added to `"ID"` ar the following.
```{r}
TS_instance["names"]
```
